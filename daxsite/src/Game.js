...
const Game = ({ wsUrl }) => {
  ...
  const [wasDisconnected, setWasDisconnected, disconnectRef] = useStateRef(false); // Track disconnection status
  ...
  const createGame = () => {
    if (isWsOpen) {
      ws.current.send(JSON.stringify({ type: 'create', playerName: user.name }));
    }
  };

  const joinGame = (gameId) => {
    if (isWsOpen) {
      ws.current.send(JSON.stringify({ type: 'join', gameId, playerName: user.name }));
    }
  };
